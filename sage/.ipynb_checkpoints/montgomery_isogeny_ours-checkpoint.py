...
def factored_kummer_isogeny(K, P, order, cP, primes, threshold=1000):
    ...
    for l, e in factors:
        ...
        if order % D == 0:
            ...
            phi_list += psi_list
            P = evaluate_factored_kummer_isogeny(psi_list, psi_list, P, dummy=False)
            # cP = evaluate_factored_kummer_isogeny(psi_list, psi_list, cP, dummy=False)
            
            
        else:         
            # Compute dummy point Q of order l^e 
            Q = primes * cP
...
class KummerLineIsogenyComposite(KummerLineIsogeny_Generic):
    ...
    def __init__(self, domain, oP, order, cP, primes, check=True, threshold=1500):
        ...
        # self.validate_input(domain, oP, order, check=check)

        # Compute factored isogeny
        self._phis, self._all_phis = factored_kummer_isogeny(domain, oP, order, cP, primes, threshold=threshold)

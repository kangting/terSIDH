...
def ct_select_point(P_old, P_new, m):
    """
    m in {0,1}
    """
    Xo, Zo = P_old.XZ(); Xn, Zn = P_new.XZ()
    
    if m == 0:
        return KummerPoint(P_old.parent(), ((1-m)*Xo + m*Xn, (1-m)*Zo + m*Zn))
    
    else:
        return KummerPoint(P_new.parent(), ((1-m)*Xo + m*Xn, (1-m)*Zo + m*Zn))

def evaluate_factored_kummer_isogeny(phi_list, m_list, P, dummy=True):
    if dummy:
        for phi, m in zip(phi_list, m_list):
            Q  = phi(P)
            P = ct_select_point(P, Q, m)
        return P
    else:
        for phi in phi_list:
            P = phi(P)
...
def factored_kummer_isogeny(K, P, order, cP, primes, threshold=1000):

    if P.parent() != K or cP.parent() != K:
    ...
    def sparse_isogeny_prime_power(P, l, e, split=0.8, threshold=1000):
        ...
        def recursive_sparse_isogeny(Q, k):
            ...
            Q2 = evaluate_factored_kummer_isogeny(L, None, Q, dummy=False)
    ...
    for l, e in factors:
        ...
        Q  = ct_select_point(cP, P, m)
        
        Q2 = primes * Q
           
        psi = sparse_isogeny_prime_power(Q2, l, e, threshold=threshold)

        Q = evaluate_factored_kummer_isogeny(psi, None, Q, dummy=False)

        P  = ct_select_point(P,  Q, m)
        cP = ct_select_point(cP, Q, 1-m)

        phi_list += psi
        m_list   += [m] * len(psi)

    return phi_list, m_list
...
class KummerLineIsogenyComposite(KummerLineIsogeny_Generic):
    def __init__(self, domain, oP, order, cP, primes, check=True, threshold=1500):
        self._phis, self._m = factored_kummer_isogeny(
            domain, oP, order, cP, primes, threshold=threshold
        )
        ...
        L = domain
        deg = 1
        for phi, m in zip(self._phis, self._m):
            if m:
                deg *= phi.degree()
                L = phi.codomain()
        self._degree   = deg
    ...
    def __call__(self, P):
        return evaluate_factored_kummer_isogeny(self._phis, self._m, P, dummy=True)
    ...
    def from_factors(cls, maps):
        ...
        for phi in maps:
            if not isinstance(phi, KummerLineIsogeny_Generic):
                raise TypeError(f'not a kummer-line isogeny: {phi}')
            if phi.domain() != L:
                raise ValueError(f'isogeny has incorrect domain: {phi}')
            L = phi.codomain()
        result = cls.__new__(cls)
        result._phis = maps
        result._all_phis = maps
        result._degree = prod(phi.degree() for phi in maps)
        result._domain = maps[0].domain()
        result._codomain = maps[-1].codomain()
        return result
